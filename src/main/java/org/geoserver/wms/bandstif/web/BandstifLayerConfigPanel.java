 ...
 */
package org.geoserver.wms.bandstif.web;
...
import org.geoserver.wms.bandstif.BandstifConfig;
 ...
 */
public class BandstifLayerConfigPanel extends PublishedConfigurationPanel<LayerInfo> {
    private static final long serialVersionUID = -7082211085566621848L;

    private static final Logger LOGGER = Logging.getLogger(BandstifLayerConfigPanel.class);

    public BandstifLayerConfigPanel(String id, IModel<LayerInfo> model) {
        super(id, model);
        ...
        add(
                new DropDownChoice<String>(
                        BandstifConfig.DEFAULT_DATA_TYPE,
                        new MapModel(metadata, BandstifConfig.DEFAULT_DATA_TYPE),
                        new ListModel<>(
                                Arrays.asList(
                                        "application/bil8",
                                        "application/bil16",
        ...
        add(
                new DropDownChoice<String>(
                        BandstifConfig.BYTE_ORDER,
                        new MapModel(metadata, BandstifConfig.BYTE_ORDER),
                        new ListModel<>(
                                Arrays.asList(
        ...
        add(
                new TextField<Double>(
                        BandstifConfig.NO_DATA_OUTPUT,
                        new MapModel(metadata, BandstifConfig.NO_DATA_OUTPUT),
                        Double.class));
    }
        ...
        public Object getDisplayValue(String str) {
            ...
            if (ByteOrder.LITTLE_ENDIAN.toString().equals(str)) {
                return new StringResourceModel(
                                "byteOrderLittleEndian", BandstifLayerConfigPanel.this)
                        .getObject();
            } else if (ByteOrder.BIG_ENDIAN.toString().equals(str)) {
                return new StringResourceModel("byteOrderBigEndian", BandstifLayerConfigPanel.this)
                        .getObject();
            }
